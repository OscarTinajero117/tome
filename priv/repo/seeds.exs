...
fields = [
  {"Designing Elixir Systems with OTP", "9781680506617", "Learn some OTP", ~D[2019-12-01], "published"},
...
books =
  Enum.map(fields, fn {title, isbn, desc, published, status} ->
    %{title: title, isbn: isbn, description: desc, status: status, published_on: published, inserted_at: now, updated_at: now}
  end)

  Tome.Repo.insert_all(Tome.Library.Book, books)

book_ids = Tome.Repo.all(from b in Tome.Library.Bool, select: b.id)
...
Tome.Repo.insert_all(Tome.Feedback.Raiting, ratings)
